...
__global__ void forwardUtil(double *nni, double *nno, double *wt, int num, DIM4 *nnD, DIM4 *wtD, DIM2 *stride) {
    ...
    //printf("idxw_j_k_ %d %d %d %d\n", idx, w_, j_, k_);
    double sum = 0;
    for(int i = 0; i < wtD[num].z; ++i) {
        for(int j = 0; j < wtD[num].x; ++j) {
            for(int k = 0; k < wtD[num].y; ++k) {
        //printf("wt vs nni %d %d %d %d\n", idx, wtD[num].z, wtD[num].y, wtD[num].x);
...
double forwardProp(double *nn, double *wt, int num, DIM4 *nnD, DIM4 *wtD, DIM2 *stride, int type, DIM4 *hnnD, DIM4 *hwtD) {
    ...
    #ifdef DEB
        double *wtdeb2 = new double[hwtD[i].x*hwtD[i].y*hwtD[i].z*hwtD[i].w];
        cudaMemcpyDTH(wtdeb2, wt, hwtD[i].x*hwtD[i].y*hwtD[i].z*hwtD[i].w*sizeof(double));
        cout << "in : " << endl;        
        for(int j = 0; j < hwtD[i].x*hwtD[i].y*hwtD[i].z*hwtD[i].w; ++j)
            cout << wtdeb2[j] << endl;
    ...
    #ifdef DEB
        double *wtdeb = new double[prevBytes2];
        cudaMemcpyDTH(wtdeb, wt, prevBytes2*sizeof(double));
        cout << "out : " << endl;       
        for(int j = 0; j < prevBytes2; ++j)
            cout << wtdeb[j] << endl;
        
    #endif
    }
    double h_maxT = 0, *d_maxT;
    cudaMalloc((void**)&d_maxT, sizeof(double));
    cudaMemcpyHTD(d_maxT, &h_maxT, sizeof(double));
...
__global__ void initFlayer(double *nn, char *x) {
    ...
    //printf("idx tij %d %d %d %d\n", idx, t, i, j);
    if(j>=80)
        nn[idx] = 0;
    else
        nn[idx] = getVal(x[t*2*210*160 + (2*i+30)*2*160 + 2*2*j], x[t*2*210*160 + (2*i+30)*2*160 + 2*2*j+1]);
...
double forwardPropMain(string x, double *nn, double *wt, int num, DIM4 *nnD, DIM4 *wtD, DIM2 *stride, int type, DIM4 *hnnD, DIM4 *hwtD) {
    ...
    */
    ...
    //cudaMemcpyDTH(fLayer, nn, hnnD[0].x*hnnD[0].y*hnnD[0].z*sizeof(double));
    //ofstream fp("cc.txt");
    //for(int i = 0; i <  hnnD[0].x*hnnD[0].y*hnnD[0].z; ++i)
    //fp << fLayer[i] << endl;
    //fp.close();
    //exit(1);
    cudaFree(d_x);
...
__global__ void updateWts(double *err, double *nn, double *wt, DIM4 *nnD, DIM4 *wtD, DIM2 *stride, int num, double *msq, int maxNum) {
    ...
    for(int i = 0; i < nnD[num+1].x; ++i) {
        ...
        //printf(" idx %d  sum  %f err %f drivv %f nn %f\n", idx, sum, err[w*nnD[num+1].x*nnD[num+1].y + i*nnD[num+1].y + j],derivv(nn[w*nnD[num+1].x*nnD[num+1].y + i*nnD[num+1].y + j]), nn[c*nnD[num].x*nnD[num].y + (i*stride[num].x + a)*nnD[num].y + j*stride[num].y+b]);
    }
    }
    ...
    //printf("idx: %d  sum: %f  msq[idx]=%f wt[idx]=%f\n", idx, sum, msq[idx], wt[idx]);
}
...
void backPropMain(double *nn, double *wt, int num,  DIM4 *nnD, DIM4 *wtD, DIM2 *stride, int type, DIM4 *hnnD, DIM4 *hwtD, int nnNumUnits, int wtNumUnits) {
    ...
    double *tempNN = new double[nnNumUnits];
    double *tempWt = new double[wtNumUnits];
    for(int i = 0; i < nnNumUnits; ++i)
    tempNN[i] = 0;
    for(int i = 0; i < wtNumUnits; ++i)
    tempWt[i] = 0;
    cudaMemcpyHTD(d_msq, tempWt, wtNumUnits*sizeof(double));
    for(int i = 0; i < backPropData.size(); ++i) {
        cudaMemcpyHTD(d_err, tempNN, nnNumUnits*sizeof(double));
        ...
        for(int j = num-1; j >= 0; --j) {
            ...
            updateWts<<<numBlocks, threadsPerBlock>>>(d_err + lBytes, nn + lBytes, wt + wtLBytes, nnD, wtD, stride, j, d_msq, hwtD[j].x*hwtD[j].y*hwtD[j].z*hwtD[j].w);
...
int main(int argc, char **argv) {
    ...
    cudaMemcpyHTD(d_wtD, h_wtD, numWT*sizeof(DIM4));
    cudaMemcpyHTD(d_stride, h_stride, numWT*sizeof(DIM2));
    ...
    for(int i = 0; i < nnNumUnits; ++i)
        nndeb[i] = 1.0;
        cudaMemcpyHTD(d_nnLayer, nndeb, nnNumUnits*sizeof(double));
    
        double x = forwardProp(d_nnLayer, d_wtLayer, numWT, d_nnD, d_wtD, d_stride, 1, h_nnD, h_wtD);

        cudaMemcpyDTH(nndeb, d_nnLayer, nnNumUnits*sizeof(double));
        cudaMemcpyDTH(wtdeb, d_wtLayer, wtNumUnits*sizeof(double));
    
    cout << "Num nn units: " << nnNumUnits << endl;
    cout << "Num wt units: " << wtNumUnits << endl; 

    cout << "nn Layer sizes: " << endl;
    for(int i = 0; i < numNN; ++i)
        cout << i << " " << h_nnD[i].x << " " << h_nnD[i].y << " " << h_nnD[i].z << " " << h_nnD[i].w << endl;
    
    cout << "wt Layer sizes: " << endl;
    for(int i = 0; i < numWT; ++i)
        cout << i << " " << h_wtD[i].x << " " << h_wtD[i].y << " " << h_wtD[i].z << " " << h_wtD[i].w << endl;
    ...
    cudaMemcpyHTD(d_msq, tempWt, wtNumUnits*sizeof(double));
    int num = numWT;
    int lBytes = nnNumUnits - h_nnD[num].x*h_nnD[num].y*h_nnD[num].z;
    int wtLBytes = wtNumUnits;
    cudaMemcpyHTD(d_err, tempNN, nnNumUnits*sizeof(double));
    setFinalLayer<<<1,1>>>(d_nnLayer + lBytes, d_err + lBytes, 0, -1);
    for(int j = numWT-1; j >= 0; --j) {
        wtLBytes -= h_wtD[j].x*h_wtD[j].y*h_wtD[j].z*h_wtD[j].w;
        dim3 threadsPerBlock(BLOCKSIZE);
        dim3 numBlocks((h_wtD[j].x*h_wtD[j].y*h_wtD[j].z*h_wtD[j].w-1)/threadsPerBlock.x + 1); 
        dim3 numBlocks2((h_nnD[j].x*h_nnD[j].y*h_nnD[j].z*h_nnD[j].w-1)/threadsPerBlock.x + 1); 
        propagateError<<<numBlocks2, threadsPerBlock>>>(d_err + lBytes, d_err + lBytes - h_nnD[j].x*h_nnD[j].y*h_nnD[j].z, d_nnLayer + lBytes, d_wtLayer + wtLBytes, d_nnD, d_wtD, d_stride, j, h_nnD[j].x*h_nnD[j].y*h_nnD[j].z*h_nnD[j].w);
        updateWts<<<numBlocks, threadsPerBlock>>>(d_err + lBytes, d_nnLayer + lBytes , d_wtLayer + wtLBytes, d_nnD, d_wtD, d_stride, j, d_msq, h_wtD[j].x*h_wtD[j].y*h_wtD[j].z*h_wtD[j].w);
        lBytes -= h_nnD[j].x*h_nnD[j].y*h_nnD[j].z;
    }

    cudaMemcpyDTH(nndeb, d_err, nnNumUnits*sizeof(double));
    cout << "err" << endl;
    for(int i = 0; i < nnNumUnits; ++i)
        cout << nndeb[i] << endl;
    cout << "nn" << endl;
    ...
    double x_ = forwardProp(d_nnLayer, d_wtLayer, numWT, d_nnD, d_wtD, d_stride, 1, h_nnD, h_wtD);
    cout << x_ << endl;
        exit(1);
    #endif
    ...
    */
    ...
    while(i < maxIter - numFrameSkip) {
        ...
        for(int kk = 0; kk < miniBatchSize; ++kk) {
            ...
            y[kk] = D[tt].rw;
            if(D[tt].isterm==0) {
                y[kk] += gamma*forwardPropFileMain(D[tt].pres, d_nnLayer, d_wtLayer, numWT, d_nnD, d_wtD, d_stride, 1, h_nnD, h_wtD);
